...
/**
 * Generate or patch a React component via DeepSeek-R1-0528.
 */
exports.generateComponent = async (prompt, history, currentCode = { jsx: "", css: "" }) => {
  const systemPrompt = `
...
Rules:
1. Modify existing code if present.
2. Return ONLY JSX & CSS in valid JSON format.
3. Use className attributes in JSX.
4. Format CSS with proper line breaks and indentation.
5. Make CSS readable with each property on a new line.
...
Example response:
{
  ...
  try {
    ...
    try {
      ...
      // Fallback: try to extract JSX and CSS manually
...
//function to format minified CSS
